...
let negocio = document.getElementById('negocioPracticanteActividad');
let negocioname = negocio.value;
...
cargarEventos()
    .then(eventos => {
        const eventos2 = eventos.map(evento => ({
            ...
            nombre_negocio: evento.nombre_negocio,
            ...
            for (let i = 1; i <= totalDays; i++) {
                ...
                const eventsForDate = eventos2.filter(evento => {
                    ...
                    return currentDate >= eventDate && currentDate <= eventDate &&
                        ((evento.tipo_publicacion === 'UNIDAD' || evento.tipo_publicacion === 'PERSONA') &&
                        (evento.tipo_publicacion !== 'PERSONA' || evento.id_empleado == actividad)&&
                        (evento.tipo_publicacion !== 'UNIDAD' || evento.nombre_negocio == negocioname));
                });
                        ...
                        <div class="calendar__day-1" data-toggle="modal" data-target="#eventoModal">
                            <div class="tarea__calendar" style="background-color: ${evento.color};"
                                ...
                                onclick="mostrarDatos(${evento.id})">
                                    ...
                                    <input type="hidden" id="negocio-${evento.id}" value="${evento.nombre_negocio}">
    ...
    function mostrarDatos(eventoId) {
        ...
        const inputNegocio = document.getElementById(`negocio-${eventoId}`);
        ...
        let negocioModal = document.getElementById("negocioModal");
        ...
        if (inputNegocio) {
            const negocio = inputNegocio.value;
            negocioModal.value = negocio;
        } 
    
        // Asegúrate de que tengas el campo imagen_url en tu formulario
